...
# Loss functions.
def pytorch_quantile_loss(y: Union[Tensor, ndarray], y_pred: Union[Tensor, ndarray], quantile: float) -> Tensor:
...
def extract_cols_from_data_type(data_type: DataTypes, column_definition: List,
...
def numpy_normalised_quantile_loss(y: Union[Tensor, ndarray], y_pred: Union[Tensor, ndarray], quantile: float) -> float:
  ...
  """
    prediction_underflow: Tensor = y - y_pred
    weighted_errors: Tensor = quantile * np.maximum(prediction_underflow, 0.) \
                      + (1. - quantile) * np.maximum(-prediction_underflow, 0.)

    quantile_loss: Tensor = weighted_errors.mean()
    normaliser: Tensor = y.abs().mean()
