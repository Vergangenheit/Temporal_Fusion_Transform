...
class GatedResidualNetwork(nn.Module):
    def __init__(self, input_size: int, hidden_state_size: int, output_size: int, dropout: float,
                 hidden_context_size: int = None,
...
class TFT(nn.Module):
    ...
    def decode(self, x: Tensor, hidden=None):
    ...
    def forward(self, x: Dict) -> (Tensor, Tensor, Tensor, Tensor, Tensor, Tensor, Tensor):
        ...
        # print(
        #     f"Sizes are {x.get('inputs').size()}, {x.get('outputs').size()}, {x.get('active_entries').size()}, {x.get('time').size()}, {x.get('identifier').size()}")
        embedding_vectors = []
        for i in range(self.static_variables):
        ...
        static_embedding: Tensor = torch.cat(embedding_vectors, dim=1)
        embeddings_encoder: Tensor = self.apply_embedding(
            x['inputs'][:, :self.encode_length, :].float().to(self.device),
            static_embedding, apply_masking=False)  # torch.Size([168, 64, 48])

        embeddings_decoder: Tensor = self.apply_embedding(
            x['inputs'][:, self.encode_length:, :].float().to(self.device),
            static_embedding, apply_masking=True)  # torch.Size([24, 64, 40])
        ...
        embeddings_encoder, encoder_sparse_weights = self.encoder_variable_selection(
            ...
            embeddings_encoder[:, :, -(self.embedding_dim * self.static_variables):])  # [168, 64, 160], [168, 64, 1, 5]
        embeddings_decoder, decoder_sparse_weights = self.decoder_variable_selection(
            ...
            embeddings_decoder[:, :, -(self.embedding_dim * self.static_variables):])  # [24, 64, 160] , [24, 64, 1, 4]
        pe = self.position_encoding(torch.zeros(self.seq_length, 1, embeddings_encoder.size(2)).to(self.device)).to(
            self.device)  # [192, 1, 160]
        embeddings_encoder: Tensor = embeddings_encoder + pe[:self.encode_length, :, :]  # [168, 64, 160]
        embeddings_decoder: Tensor = embeddings_decoder + pe[self.encode_length:, :, :]  # [24, 64, 160]

        ##LSTM
        lstm_input: Tensor = torch.cat([embeddings_encoder, embeddings_decoder], dim=0)  # [192, 64, 160]
        encoder_output, hidden = self.encode(embeddings_encoder)  # [168, 64, 160] , [1, 64, 160]
        decoder_output, _ = self.decode(embeddings_decoder, hidden)  # [24, 64, 160] , [1, 64, 160]
        lstm_output: Tensor = torch.cat([encoder_output, decoder_output], dim=0)  # [192, 64, 160]
        ##skip connection over lstm
        lstm_output: Tensor = self.post_lstm_gate(lstm_output + lstm_input)  # [192, 64, 160]
        ##static enrichment
        static_embedding: Tensor = torch.cat(lstm_output.size(0) * [static_embedding]).view(lstm_output.size(0),
                                                                                            lstm_output.size(1),
                                                                                            -1)  # [192, 64, 8]
        attn_input: Tensor = self.static_enrichment(lstm_output, static_embedding)  # [192, 64, 160]
        ##skip connection over lstm
        attn_input: Tensor = self.post_lstm_norm(lstm_output)  # [192, 64, 160]
        ...
        attn_output: Tensor = self.post_attn_gate(attn_output) + attn_input[self.encode_length:, :, :]  # [24, 64, 160]
        attn_output: Tensor = self.post_attn_norm(attn_output)  # [24, 64, 160]
        output: Tensor = self.pos_wise_ff(attn_output)  # [self.encode_length:,:,:]) [24, 64, 160]
        ##skip connection over Decoder
        output: Tensor = self.pre_output_gate(output) + lstm_output[self.encode_length:, :, :]  # [24, 64, 160]
        # Final output layers
        output: Tensor = self.pre_output_norm(output)  # [24, 64, 160]

        output: Tensor = self.output_layer(output.view(self.batch_size, -1, self.hidden_size))  # [64, 24, 3]
